...
c     CHARACTER*(24)  CHUTMJ
      ...
      INTEGER     NMEASM
      PARAMETER ( NMEASM = 50000 )
      INTEGER     NREALM
      PARAMETER ( NREALM = 5000 )
      INTEGER     NREAL
C
...
c     INTEGER       ISPARR( NRELVM )
      ...
      REAL*8        DEPOT1( NSPMAX )
      REAL*8        DEPOT2( NSPMAX )
      REAL*8        DOMPC2( NSPMAX )
c     REAL*8        DIFARR( NRELVM )
c     REAL*8        DWKARR( NRELVM )
c     REAL*8        DSXVAR( NSPMAX )
c     REAL*8        DSYVAR( NSPMAX )
      REAL*8         DTIME1
      REAL*8         DTIME2
      REAL*8         DCCVAL
c     REAL*8         DDIFF
...
C
      ...
      NIARGS  = IARGC()
      IF ( NIARGS.NE.4 ) THEN
        WRITE (6,*) 'Usage:  NSP    NREAL   EVENT1    EVENT2     '
        WRITE (6,*) '        111    2000    DPRK3      DPRK4      '
        CALL EXIT(1)
      ENDIF
C
      CHARG  = ' '
...
C
      ...
      IARG   = 2
      CALL GETARG( IARG, CHARG )
      CMESS  = 'Error reading integer NREAL'
      READ ( CHARG, *, ERR=99, END=99 ) NREAL
      IF ( NREAL.LT.1 .OR. NREAL.GT.NREALM ) THEN
        WRITE (6,*) 'NREAL  = ', NREAL
        WRITE (6,*) 'NREALM = ', NREALM
        CMESS  = 'NREALM exceeded'
        GOTO 99
      ENDIF
C
      CHARG  = ' '
...
C
 ...
 40   CONTINUE
C
C OK - we now know NSP - so let us set NTMEAS( ISP ) to zero for all ISP.
C
      NLSP   = 0
      DO ISP = 1, NSP
        NTMEAS( ISP ) = 0
...
C So we can increment NTMEAS( ISP )
C
      IF ( NTMEAS( ISP ).EQ.NMEASM ) THEN
        PRINT *,' NMEASM = ', NMEASM
        PRINT *,' ISP    = ', ISP
        CMESS  = 'NMEASM about to be exceed ... '
        GOTO 99
      ENDIF
      NTMEAS( ISP )    = NTMEAS( ISP ) + 1
      I                = NTMEAS( ISP )
      IF ( I.EQ.1 ) NLSP = NLSP + 1
      DT1BNK( ISP, I ) = DTIME1
      DT2BNK( ISP, I ) = DTIME2
      DCCBNK( ISP, I ) = DCCVAL
c     DDIFF  = DTIME2 - DTIME1
c     ISPARR( NRELVL ) = ISP
c     DEPOT1( NRELVL ) = DTIME1
c     DEPOT2( NRELVL ) = DTIME2
c     DCVARR( NRELVL ) = DCCVAL
c     DIFARR( NRELVL ) = DDIFF
c     DWKARR( NRELVL ) = DDIFF
c     DSXVAR( NRELVL ) = DSXARR( ISP )
c     DSYVAR( NRELVL ) = DSYARR( ISP )
c     DAMAT( NRELVL, 1 ) = DSXVAR( NRELVL )
c     DAMAT( NRELVL, 2 ) = DSYVAR( NRELVL )
c     DAMAT( NRELVL, 3 ) = 1.0d0
c     CRLINE( NRELVL ) = ' '
...
C
      DISTMX = 0.5d0
c     CALL GRDRLS( IERR, NRELVM, NRELVL, DEPOT1, DEPOT2,
c    1             DOMPC2, DSXVAR, DSYVAR, DCVARR, DRESVL,
c    2             DISTMX, DELXKM, DELYKM, DRESNM,
c    3             DDVEC, DWORK1, DAMAT, DTEMP1, DOLDRV )
      CALL MCGRRL( IERR, NSPMAX, NMEASM, NSP, NLSP, NTMEAS,
     1             DSXARR, DSYARR, DSXVLS, DSYVLS,
     2             DEPOT1, DEPOT2, DT1BNK, DT2BNK, DCCBNK,
     ...
     6             DT1USD, DT2USD, DCCUSD, DRESTM )
      IF ( IERR.NE.0 ) THEN
        WRITE (6,*) 'Error from MCGRRL '
        CALL EXIT(1)
      ENDIF
C
c     DO I = 1, NRELVL
c       WRITE (6,83) I, DSXVAR(I), DSYVAR(I), DDVEC( I ),
c    1                  DDVEC( I ) - DRESVL( I ), DRESVL( I ),
c    2                  CTARE1(1:LTARE1), CTARE2(1:LTARE2), 'res'
c     ENDDO
c83   FORMAT(I5,1X,f10.6,1X,f10.6,1X,f10.4,1X,f10.4,1X,f6.3,
c    1       1X,A,1X,A,1X,A)
C
c     WRITE (6,82) CTARE1(1:LTARE1),
c    1             CTARE2(1:LTARE2),
c    2             DELXKM, DELYKM,
c    3             DRESNM
