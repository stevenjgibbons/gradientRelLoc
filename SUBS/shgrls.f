C
C SHifted Gradient Relative Location Solve
C Steve Gibbons 2025/10/22
C
C Almost identical to GRDRLS except that we specify
C a basis DX0KM, DY0KM and solve for DELXKM, DELYKM
C relative to this basis value.
C That is, for each value of the Y vector, we subtract
C DTPRED where DTPRED is constructed as
C
C DTPRED(i) = DSXVLS(i)*DX0KM + DSYVLS(i)*DY0KM
C
...
C
C DX0KM  - Reference X location in km
C DY0KM  - Reference Y location in km
C
...
C  DOLDRV( NRELVM )
C
      SUBROUTINE SHGRLS( IERR, NRELVM, NRELVL, DTIME1, DTIME2,
...
C
      IF ( NRELVL.LT.4 .OR. NRELVL.GT.NRELVM ) THEN
        WRITE (6,*) 'Subroutine SHGRLS: error '
...
C       .
C       . Now calculate and subtract the predicted time
C       .
        DTPRED = DSXVLS( IRELVL )*DX0KM + DSYVLS( IRELVL )*DY0KM
        DDVEC( IRELVL ) = DDVEC( IRELVL ) - DTPRED
C       .
