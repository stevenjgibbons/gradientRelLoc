C shift_gradientRelLoc
C
C Adapted from gradientRelLoc but we specify DX0KM and DY0KM and
C use the routine SHGRLS instead of GRDRLS.
C 
...
C cp statphase.txt shift_gradientRelLoc.input
C cat CC_times.txt >> shift_gradientRelLoc.input
C shift_gradientRelLoc ${NSP} ${event1} ${event2} ${DX0KM} ${DY0KM} < shift_gradientRelLoc.input
C 
...
C
      ...
      NIARGS  = IARGC()
      IF ( NIARGS.NE.5 ) THEN
        WRITE (6,*) 'Usage:  NSP   EVENT1  EVENT2  DX0KM  DY0KM   '
        WRITE (6,*) '        111   DPRK3   DPRK4    1.0    2.0  '
        CALL EXIT(1)
      ENDIF
C
      CHARG  = ' '
      ...
      CMESS  = 'Error reading real DX0KM'
      READ ( CHARG, *, ERR=99, END=99 ) DX0KM
C
      CHARG  = ' '
      IARG   = 5
      CALL GETARG( IARG, CHARG )
      CMESS  = 'Error reading real DY0KM'
      READ ( CHARG, *, ERR=99, END=99 ) DY0KM
C
      CHARG  = ' '
      IARG   = 1
      CALL GETARG( IARG, CHARG )
...
C
     ...
     2                  CTARE1(1:LTARE1), CTARE2(1:LTARE2), 'res'
      ENDDO
 83   FORMAT(I5,1X,f10.6,1X,f10.6,1X,f10.4,1X,f10.4,1X,f12.8,
     1       1X,A,1X,A,1X,A)
C
      DXTKM  = DELXKM + DX0KM
      DYTKM  = DELYKM + DY0KM
      DIST   = DSQRT( DELXKM*DELXKM + DELYKM*DELYKM )
      WRITE (6,82) CTARE1(1:LTARE1),
     1             CTARE2(1:LTARE2),
     2             DELXKM, DELYKM,
     3             DRESNM, DXTKM, DYTKM, DIST
 82   FORMAT('Result: ',A,1X,A,1X,f10.5,1X,f10.5,1X,f12.6,1X,
     1                 f10.5,1X,f10.5,1X,f12.6)
